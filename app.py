...
# كلمات البحث حسب نوع المحتوى
IMAGE_QUERIES = {
    'quran':        ['islamic mosque sunrise','nature quran light','serene nature spiritual'],
    'quran_tafseer':['mosque architecture light','nature peaceful green'],
    'hadith':       ['roses flowers beautiful','nature calm peaceful'],
    'prophet_story':['desert sunset golden','nature light spiritual'],
    'sahabi_story': ['desert landscape golden','old mosque architecture'],
    'islamic_fact': ['nature geometric pattern','flowers colorful blooming'],
    'duaa':         ['sky clouds sunrise','nature morning light peaceful'],
    'friday':       ['mosque friday prayer','green nature peaceful'],
    'seasonal':     ['flowers spring bloom','nature colorful beautiful'],
    'motivation':   ['mountain sunrise peak','nature inspiring landscape'],
    'tips':         ['flowers garden colorful','nature green peaceful'],
    'fact':         ['nature macro closeup','colorful flowers detail'],
    'question':     ['nature curiosity explore','colorful landscape'],
    'wisdom':       ['roses beautiful red','nature calm reflection'],
    'poet_quote':   ['roses flowers romantic','nature poetic landscape'],
    'offer':        ['flowers gift beautiful','colorful nature bright'],
    'default':      ['nature flowers beautiful','roses colorful bloom']
}
...
def fetch_unsplash_image(post_type):
    """جلب صورة من Unsplash حسب نوع المحتوى"""
    try:
...
def create_post_image(text, post_type):
    ...
    try:
        ...
        # حاول تحميل خط — استخدم الافتراضي لو فشل
        try:
        ...
        for i, line in enumerate(lines):
            y = start_y + i * 65
            # ظل النص
            draw.text((SIZE[0]//2 + 2, y + 2), line, font=font_large, fill=(0,0,0,180), anchor='mm')
            # النص الأبيض
            draw.text((SIZE[0]//2, y), line, font=font_large, fill=(255,255,255), anchor='mm')

        # شريط علوي شفاف
        draw.rectangle([(0,0),(SIZE[0],80)], fill=(0,0,0,120))
        type_names = {
            'quran':'📖 آية كريمة','quran_tafseer':'📖 تفسير القرآن',
            'hadith':'🌹 حديث شريف','duaa':'🤲 دعاء اليوم',
            'friday':'🕌 جمعة مباركة','motivation':'✨ تحفيز',
            'wisdom':'💡 حكمة','poet_quote':'🎭 شعر وأدب',
            'prophet_story':'🕌 قصة نبي','sahabi_story':'⭐ قصة صحابي',
            'tips':'💡 نصيحة','fact':'📚 معلومة','islamic_fact':'📖 إسلاميات'
        }
        label = type_names.get(post_type, '✨ محتوى')
        draw.text((SIZE[0]//2, 40), label, font=font_medium, fill=(255,220,100), anchor='mm')
