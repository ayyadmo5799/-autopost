...
{hashtags}"""
        ...
        if 'choices' in data and data['choices']:
            ...
            # فلتر الكلمات الأجنبية
            if has_foreign_words(content):
                logger.warning("Foreign words detected, retrying...")
                # إعادة المحاولة مرة واحدة بتعليمات أشد
                payload['messages'][0]['content'] += "\n\nتحذير شديد: الرد السابق احتوى على كلمات أجنبية. يُمنع منعاً باتاً كتابة أي حرف غير عربي. العربية فقط."
...
def has_foreign_words(text):
    """كشف الكلمات الأجنبية في النص"""
    import re
    # أي حرف لاتيني أو كيريلي أو غيره
    foreign = re.findall(r'[a-zA-ZЀ-ӿ一-鿿぀-ゟ]+', text)
    # استثناء الإيموجي والأرقام والرموز المسموح بها
    real_foreign = [w for w in foreign if len(w) > 1]
    if real_foreign:
        logger.warning(f"Foreign words found: {real_foreign[:5]}")
        return True
    return False
