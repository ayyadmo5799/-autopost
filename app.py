...
# ── Data directory: Railway Volume أو /tmp كبديل مؤقت ──
...
def generate_content_with_claude(topic, post_type, page_name):
    ...
    api_key = settings.get('claude_api_key', '') or os.environ.get('CLAUDE_API_KEY', '')
    if not api_key:
...
@app.route('/health')
def health():
    return jsonify({'status': 'ok', 'time': datetime.now().isoformat(), 'data_dir': DATA_DIR})

@app.route('/api/settings', methods=['GET'])
def get_settings():
    settings = load_json(SETTINGS_FILE, {})
    safe = {k: v for k, v in settings.items() if k != 'claude_api_key'}
    safe['has_api_key'] = bool(settings.get('claude_api_key') or os.environ.get('CLAUDE_API_KEY'))
    return jsonify(safe)
